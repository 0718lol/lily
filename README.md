...
4. 删除进程环境中的 `OPENAI_API_KEY` 与 `CODEX_API_KEY`，避免仓库代码读取密钥。
...
模型请求失败时，任务会自动回到队列；达到 `LILY_MAX_ATTEMPTS` 后转为 `failed`，必须由人类决定是否重试。
...
测试覆盖 Responses API 文本解析、Codex JSONL 与 Token 解析、四阶段演示执行、数据库迁移、任务生命周期和暂停状态持久化。
...
下一阶段应增加工作树清理、patch 导出、可配置验证命令、任务取消、并发租约和 GitHub App。即使增加这些能力，自动合并与生产部署也应保持关闭。
